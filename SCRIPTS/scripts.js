...
function toggleHeader() {
  ...
  if (window.pageYOffset > contactBar.offsetHeight) {
      // Ocultamos la barra de contacto dinámicamente
      contactBar.style.top = `-${contactBar.offsetHeight}px`; 

      // Movemos el header más hacia arriba de forma proporcional
      header.style.top = `-${header.offsetHeight / 2}px`; 
  } else {
      // Cuando volvemos hacia arriba, mostramos la barra de contacto
      contactBar.style.top = '0';  // Vuelve a la posición original

      // Restauramos la posición original del header
      header.style.top = '0';  
  }
}
